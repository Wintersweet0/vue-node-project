<template>
    <div class="article">
        ...
        <div class="type">
            <Breadcrumb>
                ...
                <BreadcrumbItem to="/articles">
                    <Icon type="logo-buffer"></Icon> 全部文章
                </BreadcrumbItem>
                <BreadcrumbItem :to="{path: '/articleType', query: {type: article.type}}">
                    <Icon type="ios-cafe"></Icon> {{article.typename}}
                </BreadcrumbItem>
                <BreadcrumbItem :to="'/article/' + article.a_id">
                    <Icon type="ios-body"></Icon> {{article.title}}
                </BreadcrumbItem>
            </Breadcrumb>
        </div>
        <!-- 显示小标签 -->
        ...
        <div>
            ...
            <p class="article-p">
                作者：{{article.writer}} 浏览量：{{article.view}} 收藏：{{article.like}}
            </p>
            <!-- 显示html内容-->
            ...
            <!-- 收藏和点赞 -->
            <div>
                <Button @click="aLike(article.a_id, 1)" type="info" ghost>
...
export default {
    ...
    created: function(){
        if('id' in this.$route.params){
            ...
            //判断用户是否是第一次看该文章，如果是第一次就调用浏览量+1的API
